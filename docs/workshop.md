...
published: true
type: workshop
title: Multi-Agent Incident Response with Microsoft Agent Framework
short_title: Multi-Agent with MAF
description: Build a multi-agent incident response system using Microsoft Agent Framework and Azure AI Foundry. Learn structured outputs, workflow graphs, conditional routing, and human-in-the-loop patterns.
level: intermediate
authors:
  - Isha Salania
  - Kiran Panchal
...
Welcome to the Multi-Agent Incident Response Workshop. Over the next two hours you will build a multi-agent system that triages, diagnoses, and remediates production incidents using **Microsoft Agent Framework** and **Azure AI Foundry**.

**WeAreDevelopers World Congress 2026** · Berlin, Friday 10 July · 12:15 - 14:15 · Room Ulm

## What You Will Build

By the end of this workshop you will have a working **multi-agent incident response workflow** that processes production alerts end-to-end. The system consists of four specialized agents connected by a workflow graph with conditional routing and human approval gates.
...
Your completed system will include:

| Component | What It Does |
|-----------|-------------|
| **Triage Agent** | Classifies incident severity and recommends a runbook using typed Pydantic output |
| **Diagnostics Agent** | Performs root cause analysis by querying metrics, logs, and service dependencies |
| **Remediation Planner** | Generates an action plan with risk assessment and rollback strategy |
| **Verification Agent** | Confirms the fix by running health checks and smoke tests |
| **Workflow Graph** | Connects all agents with deterministic switch-case routing based on severity |
| **Human Approval Gate** | Pauses the workflow for operator review before executing dangerous actions |

You will process three incidents through this system, each triggering a different path:
...
Same code, different outcomes. Routing is deterministic, based on typed data from the Triage Agent rather than LLM interpretation.
...
No prior AI or ML experience is required. If you can write Python functions and call REST APIs, you are ready for this workshop.
...
|------|----------|----------|
| 12:15 | Introduction and Setup | 15 min |
...
[Microsoft Agent Framework (MAF)](https://github.com/microsoft/agent-framework) is an open-source framework for building AI agents and multi-agent workflows in Python and .NET. It was created by the teams behind [Semantic Kernel](https://github.com/microsoft/semantic-kernel) and [AutoGen](https://github.com/microsoft/autogen) as their direct successor, combining the best of both.

**Official resources:**
- [MAF GitHub Repository](https://github.com/microsoft/agent-framework)
- [MAF Overview (MS Learn)](https://learn.microsoft.com/agent-framework/overview/agent-framework-overview)
- [MAF User Guide](https://learn.microsoft.com/agent-framework/user-guide/overview)
- [MAF Quick Start Tutorial](https://learn.microsoft.com/agent-framework/tutorials/quick-start)
- [PyPI: agent-framework-core](https://pypi.org/project/agent-framework-core/)

## Why Multi-Agent?

A single agent handles one task at a time. For complex problems that span multiple systems and require different expertise, you need **specialized agents that collaborate** through well-defined inputs and outputs:

```text
Single Agent:                    Multi-Agent (MAF):
┌──────────────┐                 ┌────────┐    ┌──────────┐    ┌─────────┐
...
- **Typed contracts** between agents (not free text parsing)
- **Different execution paths** for different situations
- **Human approval** before executing dangerous actions
- **Retry logic** with escalation when operations fail
...
Everything is pre-installed: Python 3.11, Azure CLI, all pip packages. Click the link, wait 60 seconds, and proceed to Step 2.
...
No API keys or secrets in `.env`. The `AzureCliCredential` uses your `az login` session to acquire tokens automatically. This is **passwordless authentication**, the recommended pattern for development.

## Step 4: Verify

Open `challenge-0/challenge-0.ipynb` and run all cells. You should see all checks passing: Python version, dependencies installed, environment variables loaded, Azure CLI authenticated, and a test agent returning structured output.

<div class="warning" data-title="Stuck?">

Do not spend more than 5 minutes on setup. Raise your hand and ask a coach for help.
...
A typical agent response looks like this:
...
This output cannot be consumed reliably by downstream agents or workflow logic. If the LLM says "CRITICAL" one time and "Critical-level" the next, deterministic routing is impossible.

This is the fundamental problem that makes multi-agent systems difficult: if Agent A's output is unpredictable, Agent B cannot make reliable decisions based on it.

## The Fix: Pydantic `response_format`

MAF solves this with **structured outputs**. You define a Pydantic model, and the LLM is constrained to return valid JSON matching that schema. This is not prompt engineering; it is a hard constraint at the model inference level.
...
3. The schema is sent to GPT-4.1 as the response_format parameter
...
The `response_format` parameter uses OpenAI's [Structured Outputs](https://platform.openai.com/docs/guides/structured-outputs) feature. MAF handles the schema conversion automatically. You write a Pydantic model and the framework does the rest.

</div>

## Tools: Giving Agents Abilities

Agents can call **tools** (Python functions) to interact with external systems:
...
| **Diagnostics** | `DiagnosticsResult` (you build) | `get_metrics`, `get_logs`, `check_dependencies` | Root cause analysis |
| **Remediation** | `RemediationPlan` (you build) | `restart_pod`, `scale_service`, `flush_cache` | Action plan with risk level |
| **Verification** | `VerificationResult` (you build) | `get_health_status`, `run_smoke_test` | Confirm the fix worked |
...
- Refer to `tools/mock_infra.py` for all available tools and their signatures

<div class="tip" data-title="Success criteria">

Each agent returns a validated Pydantic object. No regex, no string parsing, just typed data you can use programmatically.
...
## The Problem: Ad-Hoc Chaining Does Not Scale
...
This works for one linear path. But production incidents need **branching**: CRITICAL goes through a full pipeline with human approval, HIGH skips approval, LOW just logs it. With manual chaining, you would write nested if/else statements that become unmaintainable as complexity grows.

## The Fix: WorkflowBuilder

MAF's `WorkflowBuilder` lets you declare your agent topology as a **directed acyclic graph (DAG)**. You define what connects to what, and the engine handles execution, message passing, and state.
...
**Class-based executors** for type-aware message handlers:
...
`ctx.set_state()` and `ctx.get_state()` are **synchronous**. Do not `await` them.
...
The key feature of `WorkflowBuilder` for this workshop: deterministic one-of-N routing based on data.
...
- CRITICAL incident routes to diagnostics, then comms
- HIGH incident routes to diagnostics, then comms
- LOW incident routes to monitor_only
...
- If the fix fails, there is no human to intervene
...
1. Workflow pauses and state becomes `IDLE_WITH_PENDING_REQUESTS`
...
4. Workflow resumes (or the tool call is blocked)

**When to use:** Individual dangerous operations such as restart, delete, or scale down.

## Pattern 2: Explicit Pause/Resume

For business decisions (not just tool calls), use `ctx.request_info()` to pause the workflow and present information to a human:
...
class ApprovalGate(Executor):
    ...
    async def on_response(self, original_request, response, ctx):
        """Must have exactly 4 parameters: self, original_request, response, ctx"""
...
**When to use:** Plan review, budget approval, escalation decisions.
...
For simpler linear flows, MAF offers `@workflow` and `@step`, which are plain Python async functions with built-in checkpointing and HITL:
...
1. Wire `ApprovalGate` to `ExecuteAction` and run with pause/resume
2. Build a tool approval workflow with an agent and an approval loop
3. (Stretch) Implement a functional `@workflow` with `ctx.request_info()`

<div class="tip" data-title="Key insight">

`@response_handler` signature must have exactly 4 parameters: `(self, original_request, response, ctx)`. A missing parameter is the most common error in this challenge.
...
- Tool approval loop handles all pending tool calls
...
| Option | Difficulty | What You Learn | Time |
|--------|-----------|-------------------|------|
| A: Workflow-as-Agent | Moderate | Hierarchical composition: workflows behave like agents | 10 min |
| B: Sub-Workflow Composition | Advanced | Nested workflows as reusable graph nodes | 15 min |
| C: OpenTelemetry Tracing | Beginner | Distributed tracing for every agent and tool call | 10 min |
| D: Parallel Fan-Out | Advanced | Concurrent investigation of multiple services | 15 min |
...
This enables **hierarchical multi-agent systems** where a top-level orchestrator delegates to specialized sub-systems.

## Option B: Sub-Workflow Composition

Extract remediation as a reusable sub-workflow. Any part of your system that needs a diagnose-remediate-verify sequence can invoke it as a single node.
...
Over the course of this workshop you built a complete multi-agent incident response system. Here is a summary of each component and the pattern it demonstrates:

| Challenge | What You Built | Pattern |
|-----------|---------------|---------|
| **Challenge 1** | Four specialized agents (Triage, Diagnostics, Remediation Planner, Verification) each returning typed Pydantic models | Structured outputs via `response_format` |
| **Challenge 2** | A workflow graph connecting all agents with switch-case routing that sends CRITICAL and HIGH incidents through diagnostics, and LOW incidents to monitoring | Declarative DAG with `WorkflowBuilder` |
| **Challenge 3** | Approval gates that pause the workflow for human review before executing dangerous tools like `restart_pod` | Human-in-the-loop with `request_info` and tool approval |
| **Challenge 4** | (Bonus) Advanced composition: workflow-as-agent, sub-workflows, tracing, or parallel fan-out | Hierarchical composition and observability |

### The End-to-End Flow

When you run an incident through the completed system, here is what happens:

1. **Ingest** parses the raw alert JSON and stores it in workflow state
2. **Triage Agent** classifies severity using `check_alert_history` and `get_runbook`, returning a typed `TriageResult`
3. **Parse and Route** extracts the severity field and emits a `RoutingDecision`
4. **Switch-Case** routes CRITICAL/HIGH to diagnostics, LOW to monitor-only
5. **Diagnostics Agent** investigates root cause using `get_metrics`, `get_logs`, and `check_dependencies`
6. **Approval Gate** (Challenge 3) pauses the workflow and presents the remediation plan to a human operator
7. **Remediation** executes the approved action (pod restart, scaling, etc.)
8. **Comms** generates the incident report and posts to Slack

Each step communicates through typed contracts. Routing is deterministic. Dangerous operations require explicit human approval.
...
| Resource | Link |
|----------|------|
| Getting Started | [python/samples/01-get-started](https://github.com/microsoft/agent-framework/tree/main/python/samples/01-get-started) |
| Agent Concepts (tools, middleware, providers) | [python/samples/02-agents](https://github.com/microsoft/agent-framework/tree/main/python/samples/02-agents) |
| Workflow Samples | [python/samples/03-workflows](https://github.com/microsoft/agent-framework/tree/main/python/samples/03-workflows) |
| Hosting (A2A, Azure Functions, Durable) | [python/samples/04-hosting](https://github.com/microsoft/agent-framework/tree/main/python/samples/04-hosting) |
| End-to-End Applications | [python/samples/05-end-to-end](https://github.com/microsoft/agent-framework/tree/main/python/samples/05-end-to-end) |
...
| This Workshop Repository | [github.com/ishasalania/maf-lab](https://github.com/ishasalania/maf-lab) |
...
**Q: Do I need ML or AI experience?**
No. You are using a pre-trained model (GPT-4.1) as a reasoning engine. The focus of this workshop is orchestration: how you wire agents together, route between them, and handle failures.

**Q: How is MAF different from LangChain, CrewAI, or AutoGen?**
MAF is the direct successor to Semantic Kernel and AutoGen, created by the same Microsoft teams. Key differences: typed outputs via `response_format` (hard constraint, not prompt engineering), first-class graph-based workflows (not chains), built-in HITL with `request_info` and tool approval, and deterministic routing via switch-case on data.

**Q: Will this work after the conference?**
Yes. Create an Azure AI Foundry project, deploy a model that supports structured outputs (GPT-4o, GPT-4.1), update `.env`, and run. The repository is self-contained.

**Q: Can I use a different model?**
Yes. Change `FOUNDRY_MODEL` in `.env`. Any model that supports structured outputs will work.
